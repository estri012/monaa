...
<div class="container-fluid">
	...
	<div class="card shadow mb-4">
		...
		<div class="card-body">
			<form action="{{ route('updateadm', $lokasi->id) }}" method="POST">
				<div class="form-row">
					@csrf
					@method('patch')
					<div class="form-group col-md-6">
						...
						@enderror</label>
							<select class="custom-select" id="inputKecamatan" name="kecamatan" onchange="kecamatanChange()">
								<option value="{{ $lokasi->kecamatan }}">{{ $lokasi->kecamatan }}</option>
					...
					<div class="form-group col-md-6">
						...
						@enderror>Desa @error('desa')
							 {{ $message }}
						@enderror</label>
						<input type="text" class="form-control" id="inputDesa" name="desa" value="{{ $lokasi->desa }}">
					</div>
				</div>
		</div>
